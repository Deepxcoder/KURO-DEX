...
# Configure logging for architectural diagnostics
...
def setup_obsidian_vault(
    ...
    if not vault_root.exists():
        logger.warning(f"Vault root does not exist. Creating directory: {vault_root}")
    ...
    for subfolder in VAULT_DIRECTORIES:
        ...
        logger.info(f"Verified folder: {dir_path}")

    scripts_dir = vault_root / "Scripts"
    config_file_path = scripts_dir / "config.json"

    # Build schema-validated config dictionary
    ...
    # Write config.json cleanly formatted with indents
    with open(config_file_path, "w", encoding="utf-8") as f:
        json.dump(config_data, f, indent=4)

    logger.info(f"Created configuration file at: {config_file_path}")
    return config_file_path


if __name__ == "__main__":
    import sys
    # Use CLI argument if provided; default to the current workspace root directory
    if len(sys.argv) > 1:
        target_vault_path = sys.argv[1]
    else:
        target_vault_path = Path(__file__).parent.resolve()
